...
class AutoTrader:
    ...
    def initialize_current_coin(self):
        ...
        if not current_coin_symbol:
            self.logger.info(f"No current coin conigured.")
            return
        
        if current_coin_symbol not in self.config.SUPPORTED_COIN_LIST:
            sys.exit("***\nERROR!\nCurrent coin must be in supported coin list\n***")
        
        self.logger.info(f"Initialize the price of current coin {current_coin_symbol}")
